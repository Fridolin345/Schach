...
public class Board extends ArrayList<Piece>{
	...
	public ArrayList<Integer[]> PrepareToDraw(){
		...
		for(Piece piece : this) {
			...
			if (piece.name == PieceName.WKING) {transformedPiece[0] = 0;
			} else if (piece.name == PieceName.WQUEEN){transformedPiece[0] = 1;
			} else if (piece.name == PieceName.WBISHOP){transformedPiece[0] = 2;
			} else if (piece.name == PieceName.WKNIGHT){transformedPiece[0] = 3;
			} else if (piece.name == PieceName.WROCK){transformedPiece[0] = 4;
			} else if (piece.name == PieceName.WPAWN){transformedPiece[0] = 5;
			} else if (piece.name == PieceName.BKING) {transformedPiece[0] = 6;
			} else if (piece.name == PieceName.BQUEEN){transformedPiece[0] = 7;
			} else if (piece.name == PieceName.BBISHOP){transformedPiece[0] = 8;
			} else if (piece.name == PieceName.BKNIGHT){transformedPiece[0] = 9;
			} else if (piece.name == PieceName.BROCK){transformedPiece[0] = 10;
			} else if (piece.name == PieceName.BPAWN){transformedPiece[0] = 11;
			} else { System.out.println("Error"); System.exit(0);
			}
			
			if(piece.column == 'a') { transformedPiece[1] = 0;
			} else if (piece.column == 'b') { transformedPiece[1] = 1;
			} else if (piece.column == 'c') { transformedPiece[1] = 2;
			} else if (piece.column == 'd') { transformedPiece[1] = 3;
			} else if (piece.column == 'e') { transformedPiece[1] = 4;
			} else if (piece.column == 'f') { transformedPiece[1] = 5;
			} else if (piece.column == 'g') { transformedPiece[1] = 6;
			} else if (piece.column == 'h') { transformedPiece[1] = 7;
			}
